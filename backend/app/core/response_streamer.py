...
def get_llm_client() -> genai.Client:
    ...
    global _client
    if _client is None:
        logger.info("Initializing Gemini client for response streaming")
...
async def _stream_llm(system_prompt: str, messages: list[dict]) -> AsyncIterator[str]:
    ...
    """
    logger.info(
        "Starting Gemini stream: messages=%s prompt_len=%s",
        len(messages),
        len(system_prompt),
    )
    ...
    for chunk in response:
        if chunk.text:
            ...
            yield chunk.text
    logger.info("Gemini stream complete: chunks_text_len=%s", total_chars)
