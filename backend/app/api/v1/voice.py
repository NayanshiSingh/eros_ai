...
from app.core.decision_engine import get_decision_token, strip_leading_decision_token
...
async def internal_voice_turn(request: VoiceTurnRequest):
    ...
    r = get_redis()
    logger.info(
        "Voice turn requested: session=%s transcript_len=%s",
        session_id,
        len(transcript),
    )
    ...
    if not prompt:
        ...
        logger.info("Voice turn prompt cache miss: session=%s", session_id)

    # Load conversation history from Redis
    raw_history = await r.lrange(session_history_key(session_id), 0, 5)
    history = raw_history if raw_history else []
    logger.info("Voice turn history loaded: session=%s turns=%s", session_id, len(history))
    ...
    logger.info("Voice turn decision: session=%s decision=%s", session_id, decision)
    ...
    if decision == "SEARCH":
        ...
        logger.info(
            "Voice turn memory retrieval: session=%s count=%s",
            session_id,
            len(memories),
        )
        ...
        stream = _stream_llm(prompt, messages)
        stream = strip_leading_decision_token(stream)
    ...
    async for chunk in stream:
        full_response += chunk
    logger.info(
        "Voice turn model response complete: session=%s response_len=%s",
        session_id,
        len(full_response),
    )
...
@router.post("/internal/session-end")
async def internal_voice_session_end(request: VoiceSessionEndRequest):
    """End an active session from the voice agent process."""
    await end_session(request.session_id)
    logger.info(f"Voice session ended internally: session={request.session_id}")
    return {"status": "ended"}
