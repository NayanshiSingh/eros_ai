...
def _build_locked_preview(content: str, limit: int = 110) -> str:
    ...
    return PersonalityResponse(
        ...
        history=[
            PersonalityHistorySnapshot(
                ...
                jungian_type=snapshot.get("jungian_type"),
...
async def get_diary(
    ...
    # Count visible and total entries
    visible_filter = [
        DiaryEntry.user_id == user_id,
        DiaryEntry.visible_to_user == True,  # noqa: E712
    ]
    total = await DiaryEntry.find(*visible_filter).count()
    total_entries = await DiaryEntry.find(DiaryEntry.user_id == user_id).count()
    locked_entries = max(total_entries - total, 0)

    locked_preview_entries = await DiaryEntry.find(
        DiaryEntry.user_id == user_id,
        DiaryEntry.visible_to_user == False,  # noqa: E712
    ).sort("page_number").limit(3).to_list()

    # Fetch paginated entries, sorted by date descending
    skip = (page - 1) * page_size
    entries = await DiaryEntry.find(*visible_filter).sort("-date").skip(skip).limit(page_size).to_list()
    ...
    return DiaryListResponse(
        ...
        locked_previews=[
            LockedDiaryPreviewResponse(
                ...
                date=entry.date,
                page_number=entry.page_number,
                preview=_build_locked_preview(entry.content),
            )
            for entry in locked_preview_entries
        ],
