...
class TestMemoryCRUD:
    ...
    async def test_update_memory_feedback_persists_to_doc(self, client: AsyncClient, seeded_user):
        create = await client.post(
            "/api/v1/memory/",
            json={"type": "cold", "content": "User loves rainy evenings"},
            headers={"Authorization": f"Bearer {seeded_user['token']}"},
        )
        mem_id = create.json()["id"]
