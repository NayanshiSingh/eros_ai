...
def _mock_livekit_token():
    ...
    mock_chain.with_room_config.return_value = mock_chain
...
class TestInternalVoiceTurnEndpoint:
    ...
    async def test_internal_voice_turn_strips_inline_no_search_prefix(
        self, client: AsyncClient, seeded_user
    ):
        """A combined NO_SEARCH+response chunk should be cleaned before returning."""
        session_id = await self._setup_voice_session(client, seeded_user)
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["response_text"] == "Hey! How are you?"
    ...
    async def test_internal_voice_turn_strips_followup_search_prefix(
        self, client: AsyncClient, seeded_user
    ):
        """A repeated routing token on the SEARCH follow-up stream should be stripped."""
        session_id = await self._setup_voice_session(client, seeded_user)
        ...
        with (
            ...
            resp = await client.post(
                "/api/v1/voice/internal/voice-turn",
                json={"session_id": session_id, "transcript_text": "Tell me again"},
            )

        assert resp.status_code == 200
        data = resp.json()
        assert data["response_text"] == "I remember that clearly."
...
class TestInternalVoiceSessionEndEndpoint:

    async def test_internal_session_end_marks_session_ended(
        self, client: AsyncClient, seeded_user
    ):
        """Internal session end should close the session without user auth."""
        with patch("app.voice.token_service.AccessToken", return_value=_mock_livekit_token()):
            resp = await client.post(
                "/api/v1/voice/token",
                headers={"Authorization": f"Bearer {seeded_user['token']}"},
        ...
        resp = await client.post(
            "/api/v1/voice/internal/session-end",
