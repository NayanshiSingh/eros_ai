...
class TestChatIntegration:
    ...
    async def test_no_search_inline_token_is_not_streamed_to_client(self, client: AsyncClient, seeded_user):
        """A combined NO_SEARCH+response chunk should be cleaned before streaming."""
        session_id = await self._setup_session(client, seeded_user)
        ...
        with patch(
            "app.core.response_streamer._stream_llm",
            self._mock_gemini_stream("NO_SEARCH\nHi Aryan!"),
        ):
            from app.core.response_streamer import stream_chat_response
            await stream_chat_response(session_id, "Hello!", mock_ws, seeded_user["user_id"])

        assert "".join(sent_chunks) == "Hi Aryan!"
    ...
    async def test_search_followup_token_is_not_streamed_to_client(self, client: AsyncClient, seeded_user):
        """A repeated routing token on the follow-up SEARCH stream should be stripped."""
        session_id = await self._setup_session(client, seeded_user)
        ...
        with patch("app.core.response_streamer._stream_llm", multi_stream):
            from app.core.response_streamer import stream_chat_response
            await stream_chat_response(session_id, "Remind me about that", mock_ws, seeded_user["user_id"])

        assert "".join(sent_chunks) == "I remember that clearly."
