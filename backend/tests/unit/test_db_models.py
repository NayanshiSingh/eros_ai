...
class TestMemoryModel:
    ...
    async def test_memory_model_feedback_defaults_to_none(self):
        """Memories should start with no feedback applied."""
        memory = Memory(
            user_id="usr_1",
            type=MemoryType.COLD,
            content="User likes late-night walks",
        )
        await memory.insert()

        fetched = await Memory.get(memory.id)
        assert fetched is not None
