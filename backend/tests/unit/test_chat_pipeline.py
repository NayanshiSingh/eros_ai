...
class TestDecisionEngine:
    ...
    async def test_decision_engine_strips_inline_no_search_prefix(self):
        stream = _mock_stream("NO_SEARCH\nHello there!")
        decision, remaining = await get_decision_token(stream)
    ...
    async def test_strip_leading_decision_token_removes_prefix_from_followup_stream(self):
        stream = strip_leading_decision_token(_mock_stream("SEARCH\nHere is the answer", " still going"))

        chunks = []
        async for chunk in stream:
            chunks.append(chunk)

        assert chunks == ["Here is the answer", " still going"]
