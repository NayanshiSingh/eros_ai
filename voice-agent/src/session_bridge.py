...
class SessionBridge:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            logger.info("Opening backend bridge client: base_url=%s", self.base_url)
    ...
    async def process_turn(
        ...
        logger.info(
            "POST /api/v1/voice/internal/voice-turn session=%s transcript_len=%s",
            session_id,
            len(transcript),
        )
        ...
        logger.info(
            "voice-turn response session=%s status=%s",
            session_id,
    ...
    async def mark_interrupted(
        ...
        client = await self._get_client()
        logger.info(
            "POST /api/v1/voice/internal/voice-interrupt session=%s turn_id=%s",
            session_id,
            turn_id,
        )
        response = await client.post(
        ...
        logger.info(
            "voice-interrupt response session=%s turn_id=%s status=%s",
            session_id,
            turn_id,
            response.status_code,
        )
    ...
    async def end_session(self, session_id: str) -> None:
        ...
        """
        client = await self._get_client()
        logger.info("POST /api/v1/voice/internal/session-end session=%s", session_id)
        response = await client.post(
            "/api/v1/voice/internal/session-end",
            json={"session_id": session_id},
        )
        logger.info(
            "session-end response session=%s status=%s",
            session_id,
            response.status_code,
        )
        response.raise_for_status()
